...
class Simulador:
    ...
    def _imprimir_estado_ciclo(self):
        ...
        # 3. Cola de Listos y Suspendidos
    ...
    def generar_reporte_estadistico(self) -> dict:
        ...
        for key, value in reporte.items():
            if "Tiempo" in key:
                table.append([key, f"{value:.2f} UT"])
            elif "Uso de CPU" in key:
                table.append([key, f"{value:.2f}%"])
            elif "Rendimiento" in key:
                table.append([key, f"{value:.2f} proc/UT"])
            else:
                table.append([key, f"{value:.2f}"])
